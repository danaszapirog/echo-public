 ...
 * 
 * Usage:
 *   npx ts-node prisma/seed-guided-questions.ts
 */
...
async function seedGuidedQuestions() {
  try {
    ...
    // Read the JSON file from the documents folder
    const filePath = path.join(__dirname, '../../documents/guided-questions-templates.json');
    ...
    for (const question of data.defaultQuestions) {
      await prisma.guidedQuestionTemplate.create({
        data: {
          ...
          options: question.options ? (question.options as any) : null, // Store as JSON
    ...
    const totalQuestions = data.defaultQuestions.length + data.categories.reduce((sum, cat) => sum + cat.questions.length, 0);
